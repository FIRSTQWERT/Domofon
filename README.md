...
./backup_basip.sh - пароли зашиты в скрипте, список адресов панелей лежит в ./basip_ip.txt, общий бэкап и бэкап настроек сохраняется в ./backup/BASIP
ВНИМАНИЕ! общий бэкап не всегда содержит корректный файл ключей!

./backup_beward.sh - пароли зашиты в скрипте, список адресов панелей лежит в ./beward.txt, бэкап ключей, кодов квартир и кода входной двери сохраняется в ./backup/BEWARD

./backup_trueip.sh - пароли находятся в файле passwords.txt, скрипт их перебирает по-порядку, список сетей для перебора лежит в файле ./nets.txt, бэкап ключей сохраняется ./backup/TRUE_IP 

./converter.sh - конвертирует файлы бэкапа панелей True-ip в БД ключей для загрузки через приложение TI-Concierge
...
При старте скрипта выводится пользовательское меню:
 ...
 ==> Введите адрес панели. Для вездехода используйте маску * (Пример 192.168.1.*): 192.168.1.*
